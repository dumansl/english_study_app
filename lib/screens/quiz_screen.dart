...
import 'dart:math'; // Rastgele seçim için eklendi
...
class _QuizScreenState extends State<QuizScreen> {
  List<int> _usedIndexes = []; // Kullanılan indekslerin kaydını tutmak için
  int _currentIndex = 0; // Mevcut sorunun indeksi
  ...
  // Rastgele bir indeks oluşturma
  void _generateRandomIndex() {
    if (_usedIndexes.length < widget.words.length) {
      Random random = Random();
      do {
        _currentIndex =
            random.nextInt(widget.words.length); // Yeni bir rastgele indeks seç
      } while (_usedIndexes
          .contains(_currentIndex)); // Eğer daha önce kullanılmışsa tekrar seç
      _usedIndexes.add(_currentIndex); // Kullanılan indekse ekle
    } else {
      _currentIndex = -1; // Tüm kelimeler sorulmuşsa
    }
  }
  ...
  void initState() {
    super.initState();
    _generateRandomIndex(); // Başlangıçta bir rastgele indeks oluştur
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (_answerController.text.trim().toLowerCase() ==
        ...
        setState(() {
          ...
          _wrongAttempts = 0;
          _showResultButton =
              false; // Doğru cevap olduğunda sonuç butonunu gizle
          _askEnglishToTurkish = !_askEnglishToTurkish;
          _generateRandomIndex(); // Rastgele yeni bir kelime seç
        });
      } else {
        setState(() {
          _isCorrect = false;
          ...
          if (_wrongAttempts >= 3) {
            _showResultButton =
                true; // 3 yanlış denemeden sonra sonucu göster butonu çıkıyor
          }
        });
      }
